...
//		if (!disabled) {
			el.setOnTouchListener(new OnTouchListener() {
				...
				public boolean onTouch(View v, MotionEvent event) {
					Log.i(TAG, "Touch: "+event.getAction()+", "+event.getAxisValue(MotionEvent.AXIS_X));
					return false;
				}
			});
			el.setOnLongClickListener(new OnLongClickListener() {
				...
				public boolean onLongClick(View v) {
					...
					ClipData dragData = new ClipData("Dragging item...", new String[]{"text/plain"}, item);
...
//		}
	...
	protected void fill(int h, Renderer renderer, JSONObject item, JSONObject config,
		...
		int emptyViewHeight = getFullHeight(element.getChildAt(element.getChildCount()-1));
		int height = getHeight(element);
		int added = (int) Math.floor((h-height)/emptyViewHeight);
//		Log.i(TAG, "fill list["+area+"] h: "+h+", added "+added+", empty "+emptyViewHeight+", height "+height+", () "+(added*emptyViewHeight)+", "+element.getTag(R.id.empty_items));
	...
	private int getHeight(ViewGroup element) {
		MarginLayoutParams params = (MarginLayoutParams) element.getLayoutParams();
