	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp)
		...
		try {
			...
			if ("data".equals(type)) {
				...
				filesAdded = DataStorage.backupData(
						DAO.getDataSource(getServletContext()), app, user, zip);
				// zip.closeEntry();
			}
			if ("file".equals(type)) {
				...
				filesAdded = FileStorage.backupFiles(
						DAO.getDataSource(getServletContext()), app, user,
						fromLong, zip);
			}
			if (filesAdded == 0) {
